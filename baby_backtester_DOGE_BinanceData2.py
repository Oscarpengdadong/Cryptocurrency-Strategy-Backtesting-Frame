...
#print(df.head())
df = df[df.index > pd.Timestamp('2021-02-01')]
...
plt.savefig(os.path.join(out_dir, "DOGE_price_with_signals2.png"))
...
plt.text(0.5, 0.8, f"Max Drawdown: {max_drawdown:.2f}",
         ...
         fontsize=10, color='green',
         verticalalignment='top')
plt.savefig(os.path.join(out_dir, "DOGE_equity_curve2.png"))
...
plt.legend()
plt.title("RSI")
plt.savefig(os.path.join(out_dir, "DOGE_rsi_curve2.png"))
