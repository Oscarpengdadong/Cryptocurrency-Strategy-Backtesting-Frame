...
#print(df.head())
df = df[df.index > pd.Timestamp('2020-10-01')]
...
df["signal"] = 0
df.loc[df["rsi"] < 18, "signal"] = 1     # RSI < 25 → BUY
df.loc[df["rsi"] > 88, "signal"] = -1    # RSI > 75 → SELL
...
plt.savefig(os.path.join(out_dir, "DOGE_price_with_signals.png"))
...
plt.text(0.5, 0.8, f"Sharpe Ratio: {sharpe:.2f}",
         transform=plt.gca().transAxes,   # use axes (0–1) coordinates
         fontsize=10, color='green',
         verticalalignment='top')
plt.savefig(os.path.join(out_dir, "DOGE_equity_curve.png"))
...
plt.legend()
plt.title("RSI")
plt.savefig(os.path.join(out_dir, "DOGE_rsi_curve.png"))
