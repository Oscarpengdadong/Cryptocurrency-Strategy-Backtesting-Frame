...
df_origin = conn.execute(query).fetchdf()
df_origin.drop(columns=['open_time', 'close_time'], inplace=True)
#print(df)

# Set index to open_standard for resampling
daily_df = df_origin.set_index('open_standard').resample('1h').agg({
    'open': 'first',
    'high': 'max',
    'low': 'min',
    'close': 'last',
...
#print(df.head())
df = df[df.index > pd.Timestamp('2018-10-01 00:00:00')]
...
df["signal"] = 0
df.loc[df["rsi"] < 5, "signal"] = 1     # RSI < 25 → BUY
df.loc[df["rsi"] > 95, "signal"] = -1    # RSI > 75 → SELL
...
trading_days = 365
years = len(equity_df) / (trading_days * 24)
annualized_return = (1 + total_return) ** (1 / max(years, 1e-9)) - 1

sharpe = (equity_df["returns"].mean() / (equity_df["returns"].std() + 1e-9)) * np.sqrt(trading_days * 24)
...
plt.savefig(os.path.join(out_dir, "ETH_1h_price_with_signals.png"))
...
plt.text(0.5, 0.8, f"Max Drawdown: {max_drawdown:.2f}",
         ...
         fontsize=10, color='green',
         verticalalignment='top')
plt.savefig(os.path.join(out_dir, "ETH_1h_equity_curve.png"))
...
plt.legend()
plt.title("RSI")
plt.savefig(os.path.join(out_dir, "ETH_1h_rsi_curve.png"))
